...
Xpred <- as.data.frame(model.matrix(~ x+y+elevation, data = grid))
# Create model matrix
preds <- interpolate_grid(X, y, Xpred = Xpred)
...
# Define breaks and labels for December 1st to January 31st
dec_jan_breaks <- c(1, 31, 62, 91)
dec_jan_labels <- c("Dec 1", "Jan 1", "Jan 31", "Feb 28")
...
df_warm_cold_sf$continuous_day <- sapply(df_warm_cold_sf$coldest_day, day_to_continuous_scale)
...
ggplot() +
  ...
  scale_color_continuous(name = "Day of Year", breaks = dec_jan_breaks, labels = dec_jan_labels,
                         trans = "reverse") +
  theme_minimal() +
  labs(title = "Coldest Day of the Year for Each Station", 
       x = "Longitude", y = "Latitude") + 
...
cold_dataset <- df_warmest_coldest[,c("longitude", "latitude", "rescaled_coldest")]
...
graph_interp(warm_pred, grid) + 
  labs( title = "Interpolated Warmest Day of the Year",
    x = "Longitude", y = "Latitude", 
    fill = "Day of Year") +
scale_fill_gradient(name = "Day of Year", low = "blue", high = "red")
...
graph_interp(cold_pred, grid) + 
  labs( title = "Interpolated Coldest Day of the Year",
    ...
    fill = "Day of Year") + 
#scale_fill_gradient(name = "Day of Year", low = "blue", high = "red")
  scale_fill_gradient(name = "Day of Year", breaks = dec_jan_breaks, labels = dec_jan_labels,
...
temp_trends <- sapply(station_info$station_id, trend_of_temps)

# Create a new data frame with station ids, temperature trend, lat and long
station_trends <- data.frame(station_id = colnames(temp_trends), 
...
where $(\beta_1)_i$ denotes the slope coefficient for ith month, and is derived 
from the model:
...
roughly 0.88°C per decade. However, this is largely due to the fact that we are 
only considering the temperature trends since 2000, where temperature has been 
increasing even faster. Additionally, it is the case that the vast majority of 
the US is increasing at a rate lower than 0.88°C per decade, while stations in 
the Southwest are increasing at a much greater rate. 
